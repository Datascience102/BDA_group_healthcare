...
Surgeries <- read.csv("Data/CBS - surgery outpatient and inpatient per gender and age - 1995 - 2010_v2.csv", header=TRUE, sep=",",dec=".",stringsAsFactors = FALSE)
...
Surgeries[,cols] = apply(Surgeries[,cols], 2, function(x) (as.numeric(as.character(x), na.rm = TRUE)))
...
#**Part 4.** Calculations
##Preparation of data
[...Text explaining what happens here...]
...
Consultation_imaging_ref $Activities_con= (Consultation_imaging_ref$Ratio)*sum(Consultation_1)*(Consultation_imaging_ref$Ratio) # Create proxy for number of activities per agegroup and gender based on Outpatient ratio 
...
Consultation_imaging_ref $Activities_img= (Consultation_imaging_ref$Ratio)*sum(Imaging_1)*(Consultation_imaging_ref$Ratio) # Create proxy for number of activities per agegroup and gender based on Outpatient ratio
...
colnames(Imaging_2) <- c("Age_gender_ID","Age", "Gender", "Number_of_act","Type_Act") # rename columns in order to match with other type of activities
...
```{r, Surgery_projection, eval=FALSE}


surgery_projection <- merge(Pop_index, Surgeries_def,  by.x=c("Age_gender_ID"), by.y=c("Age_gender_ID", "Type_act"),select=c("Year","Region", "Population_x1000", "Act_per_1000inhab","Space_per_act"))
...
##6.	Calculations:
=======
>>>>>>> 05210e54bb2618d26bb7359a6ef7aa38934d9be1
