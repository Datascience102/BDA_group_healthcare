...
Adm_inp2
Adm_inp3 <- aggregate(Number_of_act ~ Age_gender_ID,Adm_inp2, sum,drop=FALSE)                                  
                                  #Pop_index <- aggregate(Population_x1000 ~ .,Pop_index_temp, sum)
                                  
                                  
Adm_inp$Type_act="Nursing_days" # add column
Adm_inp <- Adm_inp[,c("Age_gender_ID","Age", "Gender", "Number_of_act","Type_act")]
Adm_inp$Space_per_act = BuildReq[which(BuildReq$Englisch.name.activity.cluster == "Nursing days"),"Total.normative.space"]
...
Adm_outp$Type_act="Outpatient_admissions" # add column
Adm_outp <- Adm_outp[,c("Age_gender_ID","Age", "Gender", "Number_of_act","Type_act")]
Adm_outp$Space_per_act = BuildReq[which(BuildReq$Englisch.name.activity.cluster == "Day admissions"),"Total.normative.space"]

Admissions_4 <- rbind(Adm_inp,Adm_outp)
