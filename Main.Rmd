...
Projection <- merge(subset(Pop_index, select=c(Age_gender_ID,Year,Region,Population_x1000 ),drop=TRUE), subset(Total_activities_2014, select=c(Age_gender_ID,Age,Gender,Act_per_1000inhab, Space_per_act),drop=TRUE), by="Age_gender_ID")
...
Projection<- subset(Projection,subset=c(Age_gender_ID,Year,Region, Space_per_act))
head(Projection,5)
```
...
```{r, Projection, eval=TRUE}
Result <-aggregate(cbind(X2, X3) ~ X1, data=df, FUN=sum)
